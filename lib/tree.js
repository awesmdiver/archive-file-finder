...
// Converts the flat entry list from `7z l -slt` (paths like
// "Folder\Sub\file.esp") into a nested tree the UI can render as
// collapsible <details> elements. Directories sort before files, both
// alphabetically.
function buildTree(entries) {
  ...
  for (const e of entries) {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      // A path can be listed as a directory entry AND be an ancestor of
      // deeper files; if we first created it as a dir via traversal and
      // later see it's not last-segment-file, that's already handled above.
